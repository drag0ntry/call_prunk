...
while(tokey2.length < 25){
	...
	function get_rights(num) {
		var rigl4 = ["Пользователь", "Вип", "Модератор", "Администратор", "Тех. Админ.", "Владелец"];
		return rigl4[num];
		}
...
function kb(mas){
var obj0 = {one_time: false, buttons: [], inline: false};
mas.map(x=> {
obj0.buttons.push([]);
x.map(z=> {
obj0.buttons[mas.indexOf(x)].push({action: {type: "text", label: z.text}, color: z.color})
});
});
return obj0;
};
